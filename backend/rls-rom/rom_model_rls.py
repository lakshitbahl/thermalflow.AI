...
class RlsRom:
    ...
    def hold(self):
        """Freeze adaptation (closed-loop guardrail): report current params, do NOT update.
        In ACTIVE mode u=MPC(x), so the regressor is correlated with the equation error
        (closed-loop identification bias). Adapting here would poison eps/U_scale. We hold."""
        self.frozen = True
        return self._diag(0.0, 0.0, gated=True)
